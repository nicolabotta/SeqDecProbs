...
> -- Pred {t} x x'  =  Exists (\ y => x' `SeqDecProbsCoreAssumptions.Elem` step t x y)
...
> dElem : {t : Nat} -> (x : State t) -> (mx : M (State t)) -> Dec (x `SeqDecProbsCoreAssumptions.Elem` mx)
> dElem x (Id x') = decEqLTB x x'

> {-
> dPred : {t : Nat} -> (x : State t) -> (x' : State (S t)) -> Dec (Pred {t = t} x x')
> dPred {t} x x' = finiteDecLemma (fCtrl t x) d1Elem where
>   d1Elem : Dec1 (\ y => x' `SeqDecProbsCoreAssumptions.Elem` (step t x y))
