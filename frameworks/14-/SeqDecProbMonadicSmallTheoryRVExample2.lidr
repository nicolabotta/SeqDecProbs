...
> nefY : (t : Nat) -> (x : X t) -> CardNotZ (fY t x)
> nefY t x = cardNotZLemma (fY t x) (existsAdmissible t x)



** Transition function:

> SeqDecProbMonadicSmallTheoryRV.step t (MkSigma Z prf) (MkSigma Left aL) =
>   Id (MkSigma maxColumn (ltIdS maxColumn))
> SeqDecProbMonadicSmallTheoryRV.step t (MkSigma (S n) prf) (MkSigma Left aL) =
>   Id (MkSigma n (ltLemma1 n nColumns prf))
> SeqDecProbMonadicSmallTheoryRV.step t (MkSigma n prf) (MkSigma Ahead aA) =
>   Id (MkSigma n prf)
> SeqDecProbMonadicSmallTheoryRV.step t (MkSigma n prf) (MkSigma Right aR) with (decLT n maxColumn)
...
>   s1  : Viable {t = S t} n x'
>   s1  = ()
>   s2  : SeqDecProbMonadicSmallTheoryRV.All (Viable {t = S t} n) mx'
>   s2  = s1
>   s3  : Exists {a = Y t x} (\ y => SeqDecProbMonadicSmallTheoryRV.All (Viable {t = S t} n) mx')
>   s3  = Evidence y s2
> ---}
...
> --{-
> neYAV t n x v = 
>   cardNotZLemma {A = Sigma (Y t x) (\ y => SeqDecProbMonadicSmallTheoryRV.All (Viable {t = S t} n) (step t x y))}
>                 (fYAV t n x v) (MkSigma y av) where
>     yav : Exists {a = Y t x} (\ y => SeqDecProbMonadicSmallTheoryRV.All (Viable {t = S t} n) (step t x y))
>     yav = viableSpec1 {t = t} {n = n} x v            
>     y   : Y t x
>     y   = getWitness yav
>     av  : SeqDecProbMonadicSmallTheoryRV.All (Viable {t = S t} n) (step t x y)
>     av  = getProof yav
> ---}
...
>        Finite1 P -> (mx : Identity (X t)) -> Finite (SeqDecProbMonadicSmallTheoryRV.All P mx)
> fAll f1P (Id x) = f1P x
...
>   ---}
>   --{
>   fViable t  n    x  = finiteUnit
>   ---}

>   f1AllViable : (t : Nat) -> (n : Nat) -> (x : X t) ->
>                 Finite1 (\ y => SeqDecProbMonadicSmallTheoryRV.All (Viable {t = S t} n) (step t x y))
...
>   Opt.max {A = Sigma (Y t x) (\ y => SeqDecProbMonadicSmallTheoryRV.All (Viable {t = S t} n) (step t x y))} 
>           {B = Nat} 
>           totalPreorderNatLTE 
>           (fYAV t n x v) 
>           (neYAV t n x v)
...
>   Opt.argmax {A = Sigma (Y t x) (\ y => SeqDecProbMonadicSmallTheoryRV.All (Viable {t = S t} n) (step t x y))} 
>              {B = Nat}
>              totalPreorderNatLTE 
>              (fYAV t n x v) 
>              (neYAV t n x v)
...
> dAll : {t : Nat} -> (P : X t -> Prop) -> Dec1 P -> (mx : M (X t)) -> Dec (SeqDecProbMonadicSmallTheoryRV.All P mx)
> dAll P dP (Id x) = dP x
