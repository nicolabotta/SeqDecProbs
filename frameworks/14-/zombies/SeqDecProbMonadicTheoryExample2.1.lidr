...
Thus, the problem is that of implementing |fYAV|. This is done in
section "Max and argmax deferred". With |fYAV| and |neYAV| we can easily
implement |max|, |argmax|, |maxSpec| and |argmaxSpec|:
...
* Max and argmax deferred

The problem here is implementing |fYAV|. We already know that |Y t x| is
finite. If we manage to show that for every |y|, |All (Viable n) (step t
x y)| is also finite, we can apply |finiteSigmaLemma| from
