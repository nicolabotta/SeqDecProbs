...
>   s1 : so (isAnyBy (\ mx => MareAllTrue (Mmap (viable n) mx)) (succs x))
>   s1 = v
>   s2 : (mx' : M (X (S t)) **
>         (so (MareAllTrue (Mmap (viable n) mx')),
>          so (mx' `isIn` (succs x))
>         )
>        )
...
>   s6 : Y t x
>   s6 = outl s5
>   s7 : so (MareAllTrue (Mmap (viable n) s3))
>   s7 = fst (outr s2)
>   s8 : s3 = step t x s6
>   s8 = outr s5
>   s9 : so (MareAllTrue (Mmap (viable n) (step t x s6)))
>   s9 = leibniz (\ xSt => so (MareAllTrue (Mmap (viable {t = S t} n) xSt))) s8 s7
>   s11 : (yy : Y t x ** 
>          so (MareAllTrue (Mmap (viable n) (step t x yy))))
>   s11 = (s6 ** s9)
...
>   mx' = step t x y
>   p : M (X (S t)) -> Bool
>   -- p = \ mx => MareAllTrue (Mmap (viable n) mx)
>   p = \ mx' => MareAllTrue (Mmap (viable n) mx')
>   pmx' : so (MareAllTrue (Mmap (viable n) mx'))
>   pmx' = outr yv
>   mxs' : (n : Nat ** Vect n (M (X (S t))))
>   mxs' = succs x
>   mx'isInmxs' : so (mx' `isIn` mxs')
>   mx'isInmxs' = succsSpec1 x y
>   step4 : so (isAnyBy  (\ mx => MareAllTrue (Mmap (viable n) mx)) mxs')
>   step4 = lemma3 mx' p (succs x) pmx' mx'isInmxs'
